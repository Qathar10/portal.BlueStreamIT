...
$(document).ready(function () {
    ...
    for (var i = 0; i < idsToProcess.length; i++) {
        ...
        if ($(id).length) {
            ...
            var isInsideModal = modalsToCheck.some(function (modalId) {
                return $(id).closest(modalId).length > 0;
            });
            ...
            if (result['error'] == true) {
                ...
                if ($('.empty-state').length > 0) {
                    if (result.hasOwnProperty('message')) {
                        ...
                        setTimeout(handleRedirection, toastTimeOut);
                    } else {
                        handleRedirection();
                    }
                } else {
                ...
                inputFields.each(function () {
                    ...
                    if (errors && errors[fieldName]) {
                        if (inputField.attr('type') !== 'radio' && inputField.attr('type') !== 'hidden') {
                            ...
                            if (inputField.hasClass('js-example-basic-multiple') || inputField.hasClass('selectTaskProject')) {
                                inputField.parent().find('.text-danger.error-message').remove();
                                inputField.parent().append(errorMessageElement);
                        ...
                        if (inputField.hasClass('js-example-basic-multiple')) {
...
$(document).ready(function () {
    ...
    $('.selectTaskProject[name="project"]').on('change', function (e) {
        ...
        var usersSelect = currentModal.find('.js-example-basic-multiple[name="user_id[]"]');
...
$(document).on('click', '.edit-project', function () {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            $('#project_priority_id').val(response.project.priority_id ? response.project.priority_id : 0).trigger('change')
...
$(document).ready(function () {
    $('select[name="priority_id"]').change(function () {
        var selectedOption = $(this).find('option:selected');
        var selectedColorClass = selectedOption.attr('class').split(' ')[1]; // Assuming the second class is the color class

        // Remove existing color classes and add the new one
        $(this).removeClass(function (index, className) {
            return (className.match(/(^|\s)bg-label-\S+/g) || []).join(' ');
        }).addClass(selectedColorClass);
    });
});
